...
class Server(BaseModel):
    """服务器数据模型"""
    id: str = Field(default="", description="服务器唯一ID")
    name: str = Field(default="", description="服务器名称")
    host: str = Field(default="", description="IP地址或主机名")
    port: int = Field(default=22, description="SSH端口")
    ssh_user: str = Field(default="root", description="SSH用户名")
    location: str = Field(default="", description="位置描述，如 阿里云/本机")
    status: str = Field(default="offline", description="状态: online/offline")
...
CREATE TABLE IF NOT EXISTS agents (
    ...
    avatar_color INTEGER DEFAULT -9868952,
